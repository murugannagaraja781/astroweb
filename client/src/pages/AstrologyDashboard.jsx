...
import { Search, MapPin, Calendar, Clock, Heart, Star, Activity } from 'lucide-react';
...
const PlaceSearch = ({ value, onChange, onSelect }) => {
  ...
  const handleSearch = async (q) => {
    ...
    if (q.length > 2) {
      try {
        const res = await axios.get(`${API_URL}/api/horoscope/places?q=${q}`);
...
const ChartForm = ({ onSubmit, title = "Enter Birth Details" }) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 bg-white p-6 rounded-xl shadow-sm border border-gray-100">
      ...
      <button type="submit" className="w-full bg-purple-600 text-white py-2 rounded-lg font-semibold hover:bg-purple-700 transition-colors">
        Generate
      </button>
    </form>
  );
...
const AstrologyDashboard = () => {
  ...
  const handleMatch = async () => {
    ...
    setLoading(true);
    try {
      const res = await axios.post(`${API_URL}/api/horoscope/match`, { male: maleData, female: femaleData });
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8 pb-24">
      <div className="container mx-auto px-4 max-w-4xl">
        ...
        <p className="text-gray-600 mb-8">Generate detailed horoscopes and check compatibility</p>
        ...
        <div className="flex gap-4 mb-6">
          <button
            ...
            className={`px-6 py-2 rounded-full font-semibold transition-all ${
              activeTab === 'chart' ? 'bg-purple-600 text-white' : 'bg-white text-gray-600 border'
            }`}
          >
            Generate Chart
          </button>
          <button
            ...
            className={`px-6 py-2 rounded-full font-semibold transition-all ${
              activeTab === 'match' ? 'bg-pink-600 text-white' : 'bg-white text-gray-600 border'
            }`}
          >
            Match Making
          </button>
        ...
        {activeTab === 'chart' && (
          <div className="grid md:grid-cols-2 gap-8">
            <div>
              <ChartForm onSubmit={handleGenerate} title="Enter Birth Details" />
            </div>
            <div>
              {loading && <div className="text-center py-10">Loading...</div>}
              {chartData && !loading && (
                <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                  <h3 className="text-xl font-bold text-gray-800 mb-4">Horoscope Details</h3>
                  <div className="mb-4">
                    <p className="text-sm text-gray-500">Name: <span className="font-medium text-gray-800">{chartData.name}</span></p>
                    <p className="text-sm text-gray-500">Date: <span className="font-medium text-gray-800">{chartData.input.date} {chartData.input.time}</span></p>
                    <p className="text-sm text-gray-500">Place: <span className="font-medium text-gray-800">{chartData.input.place.place}</span></p>
                  </div>

                  <div className="mb-6">
                    <h4 className="font-semibold text-purple-700 mb-2">Planetary Positions</h4>
                    <PlanetTable planets={chartData.planets} />
                  </div>

                  <div className="mb-6">
                    <h4 className="font-semibold text-purple-700 mb-2">Panchangam</h4>
                    <div className="grid grid-cols-2 gap-2 text-sm">
                      <div className="bg-gray-50 p-2 rounded">
                        <span className="text-gray-500 block">Tithi</span>
                        <span className="font-medium">{chartData.panchangam.tithi.tithiApprox}</span>
                      </div>
                      <div className="bg-gray-50 p-2 rounded">
                        <span className="text-gray-500 block">Nakshatra</span>
                        <span className="font-medium">{chartData.panchangam.nakshatra.name}</span>
                      </div>
                    </div>
                  </div>

                  <div>
                    <h4 className="font-semibold text-purple-700 mb-2">Current Dasa</h4>
                    <div className="bg-purple-50 p-3 rounded-lg text-purple-800 font-medium">
                      {chartData.dasa.current?.planet} Dasa (Balance: {chartData.dasa.current?.years.toFixed(1)} years)
                    </div>
                  </div>
                </div>
              )}
          ...
          <div className="space-y-8">
            ...
            <div className="text-center">
              <button
                ...
                className={`px-8 py-3 rounded-full font-bold text-lg shadow-lg transition-transform hover:scale-105 ${
                  maleData && femaleData ? 'bg-pink-600 text-white' : 'bg-gray-300 text-gray-500 cursor-not-allowed'
                }`}
              >
                Check Compatibility
              </button>
            ...
            {matchResult && (
              <div className="bg-white p-8 rounded-2xl shadow-lg border border-pink-100 text-center animate-fadeIn">
                ...
                <h3 className="text-2xl font-bold text-gray-800 mb-2">Compatibility Score</h3>
                <div className="text-5xl font-black text-pink-600 mb-4">{matchResult.score} / 2</div>
                <p className="text-xl font-medium text-gray-600 mb-6">{matchResult.recommendation}</p>

                <div className="grid grid-cols-2 gap-4 max-w-md mx-auto text-left">
                  <div className="bg-pink-50 p-4 rounded-xl">
                    <span className="text-gray-500 text-sm block">Dina Porutham</span>
                    <span className={`font-bold ${matchResult.porutham.din ? 'text-green-600' : 'text-red-500'}`}>
                      {matchResult.porutham.din ? 'Good' : 'Bad'}
                    </span>
                  </div>
                  <div className="bg-pink-50 p-4 rounded-xl">
                    <span className="text-gray-500 text-sm block">Gana Porutham</span>
                    <span className={`font-bold ${matchResult.porutham.gana ? 'text-green-600' : 'text-red-500'}`}>
