...
const Chat = () => {
  ...
  useEffect(() => {
    ...
    // Join chat room
    // For simplicity, room ID is sorted user IDs, but backend might return a specific ID
  ...
  const sendMessage = (e) => {
    ...
    if (message.trim()) {
      const msgData = {
        roomId: chatId,
        senderId: user.id,
        receiverId: receiverId, // Add receiverId explicitly
        text: message,
        timestamp: new Date(),
        type: 'text'
      };
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {messages.map((msg, index) => (
          ...
          >
            ...
            >
              {msg.type === 'audio' ? (
                <AudioMessage audio={msg.audio || msg.mediaUrl} duration={msg.duration} />
              ) : (
                <p>{msg.text}</p>
              )}
